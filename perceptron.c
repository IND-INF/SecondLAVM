...
int fit(Perceptron *perc, Matrix *xTrain, Matrix *yTrain, Matrix *xVal, Matrix *yVal,
    ...
    size_t columnsTrain[columnNumber];
    ...
    double lfSubserv;
    for(size_t epochNumber = 0, batchIndex, batchStart, batchColumn, batchRow; epochNumber < epochs; ++epochNumber){

        shuffleColumns(columnsTrain, columnNumber);
        ...
        for(batchIndex = 0; batchIndex < batchNumber; ++batchIndex){
            ...
            lossesTrain[epochNumber*batchNumber + batchIndex] = loss;
        ...
        for(batchIndex = 0; batchIndex < valBatchNumber; ++batchIndex){
            ...
            computeLoss(yBatch, yPred, &loss);
            lossesVal[epochNumber*valBatchNumber + batchIndex] = loss;
