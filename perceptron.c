...
int allocatePerceptron(Perceptron **perc, size_t weightNumber){
    ...
    if(!allocateMatrix(&((*perc)->w), 1, weightNumber)){
        free(perc);
        return 0;
    }
...
void generatePerceptron(Perceptron *perc, time_t seed){
    ...
    for(size_t number = 0; number < weightNumber; ++number){
        setMatrixElement(perc->w, 0, number, ((double)rand())*2/RAND_MAX - 1);
    }
...
double sigmoid(double x){
    if(x < -100) return 1.0/(1.0 + exp(100));
    if(x > 100) return 1.0/(1.0 + exp(-100));
...
int fit(Perceptron *perc, Matrix *xTrain, Matrix *yTrain, Matrix *xVal, Matrix *yVal,
    ...
    Matrix *totalLossDerRespW;
    if(!allocateMatrix(&totalLossDerRespW, 1, rowNumber)){
    ...
    size_t *columnsTrain;
    if(!(columnsTrain = malloc(trainColumnNumber*sizeof(size_t)))){
        freeMatrix(totalLossDerRespW);
        freeMatrix(lossDerRespW);
        freeMatrix(yPred);
        freeMatrix(yBatch);
        freeMatrix(xBatchTranspose);
    ...
    for(number = 0; number < trainColumnNumber; ++number){
    ...
    for(size_t epochNumber = 0, batch, batchStart, batchColumn, batchRow; epochNumber < epochs; ++epochNumber){
        ...
        totalLoss = 0;
        totalLossDerRespB = 0;
        for(number = 0; number < rowNumber; ++number){
            setMatrixElement(totalLossDerRespW, 0, number, 0);
        }
        ...
        for(batch = 0; batch < trainBatchNumber; ++batch){
            ...
            scalarlyDivideAndAssignMatrix((double)batchSize, lossDerRespW);
            ...
            lossDerRespB /= (double)batchSize;

            addAndAssignMatrix(totalLossDerRespW, lossDerRespW);
            totalLossDerRespB += lossDerRespB;
            
        }

        scalarlyMultiplyAndAssignMatrix(lr/trainBatchNumber, totalLossDerRespW);
        subtractAndAssignMatrix(perc->w, totalLossDerRespW);

        perc->b -= totalLossDerRespB*lr/trainBatchNumber;
...
int predict(const Perceptron *perc, const Matrix *x, int *res){
    ...
    if(!multiplyMatrices(perc->w, x, ans)){
        freeMatrix(ans);
        return 0;
    }
