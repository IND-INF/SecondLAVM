...
void scalarlyDivideAndAssignMatrix(double scalar, Matrix *mat){
...
int addAndAssignMatrix(Matrix *leftMat, const Matrix *rightMat){
...
int multiplyMatrices(const Matrix *leftMat, const Matrix *rightMat, Matrix *res){
    ...
    size_t factorNumber = leftMat->columnNumber;
    double element, factor1, factor2;
    for(size_t row = 0, column, number; row < resRowNumber; ++row){
        for(column = 0; column < resColumnNumber; ++column){

            element = 0;
            for(number = 0; number < factorNumber; ++number){
                factor1 = leftMat->values[row][number];
                factor2 = rightMat->values[number][column];
